...
import { InjectiveWatcher } from '../InjectiveWatcher';
...
test('getFinalizedBlockNumber(injective)', async () => {
  const watcher = new InjectiveWatcher();
  const blockNumber = await watcher.getFinalizedBlockNumber();
...
test('getMessagesForBlocks(injective)', async () => {
  ...
  expect(vaasByBlock['24905509/2023-01-27T19:11:35.174Z'][0]).toEqual(
    '80AE3EA576506327882EC97C34B6C1D17B3C1356AC2BA673B5157FC76364D319:19/00000000000000000000000045dbea4617971d93188eda21530bc6503d153313/33'
  );
});
