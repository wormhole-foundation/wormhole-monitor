...
//   var sql_query = `SELECT distinct source_hash as hash FROM ${tableName} WHERE chain_id=${chain} and signed_vaa is null`;
...
(async () => {
  ...
  if (startBlock != undefined) {
    ...
    //     .then(function(results){
    //         console.log(results)
    //         lastBlockNumber = results[0]['source_block']
  ...
  var transfers = [];
  var completes = [];
  [transfers, completes] = await runAlgorand(transactionArray, CHAIN_INFO_MAP[chain]);
