...
const lastTimestamp = process.env.lastTimestamp || undefined;
...
(async () => {
  ...
  if (loadFromFile != '0') {
    ...
    transactionArray = await getTxns(CHAIN_INFO_MAP[chain], lastTxnHash, lastTimestamp);
  }
  ...
  var transfers = [];
  var completes = [];
  [transfers, completes] = await runNear(transactionArray, CHAIN_INFO_MAP[chain]);
